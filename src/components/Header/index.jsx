      ...
      <div className="container">
            ...
            {categories.data?.map((category) => (
              <li className="nav-item" key={category.slug}>
                <Link
                  ...
                  className={classNames('nav-link', { active: category.slug === currentCategory.data?.slug })}
                >
                  {category.name}
                </Link>
              </li>
        ...
        <div className="mx-auto order-0">
          <div className="navbar-brand mx-auto">
            <Link
              to="/"
              className="logo m-0 text-uppercase text-decoration-none fs-2 text-dark fw-bold"
            >
              Catri.us
            </Link>
          </div>
          ...
          <ul className="navbar-nav ms-auto">
            {pages.data?.map((page) => (
              <li className="nav-item" key={page.slug}>
                <Link
                  to={`/page/${page.slug}`}
                  className={classNames('nav-link', { active: page.slug === currentPage.data?.slug })}
                >
                  {page.title}
                </Link>
              </li>
